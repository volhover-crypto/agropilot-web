...
# Mount: app.include_router(calendar_router, prefix="/agropilot/api/v1")
...
async def create_event(body: EventCreate, db: AsyncSession = Depends(), user = Depends()):
      ev = CalendarEvent(
                ...
                start_at=body.start_at, end_at=body.end_at, all_day=body.all_day,
                deal_id=body.deal_id, kind=body.kind,
                owner_id=str(user.id), owner_name=getattr(user, "name", str(user.id)),
            )
      db.add(ev)
      await db.commit()
      await db.refresh(ev)
...
async def update_event(event_id: str, body: EventPatch, db: AsyncSession = Depends(), user = Depends()):
...
async def delete_event(event_id: str, db: AsyncSession = Depends(), user = Depends()):
      ev = await db.get(CalendarEvent, event_id)
    if not ev:
              raise HTTPException(404, "Not found")
          if ev.owner_id != str(user.id):
