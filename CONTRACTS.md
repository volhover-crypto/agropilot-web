...
## §38. Справка в Медиа-мониторинге (онтология + механика)

Статус: реализовано 2026-09-17 (только фронт, c68b3c3).

- Кнопка-вкладка «📖 Справка» в строке фильтров раздела «Медиа-мониторинг»
  (newsState.help) — переключает ленту на справку и обратно.
- Содержание (vNewsHelp): онтология раздела карточками (Источник,
  Материал, Статусы, Агент A1, Сегмент аудитории) + схема потока данных
  «Источник → A1 → Материал → В пост → A2 → TG-согласование → A3 →
  Календарь»; механика пользования в 6 шагов (подключение источника,
  скан, чтение карточки с hover-превью, разбор, «В пост», удаление
  источника); подсказки (SLA при релевантности ≥70%, ключевые слова,
  fallback на слова стратегии, дашборд агентов).

## §37. Сегменты аудитории и рубрикатор для A2 (гэп №2 концепта SSM)

Статус: реализовано 2026-09-17 (миграция 035). Решения владельца 17.09:
удаление сегмента ЗАПРЕЩЕНО, пока используется (422 со списком
источников/каналов/новостей/постов — сначала отвязать); рубрики —
управляемый справочник, как сегменты; тексты сидов — по концепту apilot92.
...
- CRUD /v1/segments, /v1/segments/rubrics — право контент-мейкера
  ...
  /{segment_id} (матчинг FastAPI). DELETE сегмента — запрет при
  использовании (см. решения владельца выше); DELETE рубрики — посты
  отвязываются (rubric_code -> NULL) со счётчиком.
