...
> **АКТУАЛЬНОЕ СОСТОЯНИЕ на 2026-08-28** (ниже по файлу — журнал сессий,
> записи датированы и отражают состояние НА МОМЕНТ записи; июльские строки
> про порт 5555 и `agropilot.service` историчны, не переписывались):
>
> - Backend: `agropilot-backend.service`, venv, **127.0.0.1:5560**
>   (переехал с 5555 28.08 — порт занял `agropilot-bff.service` соседнего
>   проекта; см. запись «ERP переведён с порта 5555 на 5560»).
> - nginx: `/agropilot/api/` → `127.0.0.1:5560`, файл
>   `/etc/nginx/sites-enabled/agropilot-web` (НЕ симлинк, разошёлся с
>   `sites-available` — править надо enabled).
...
> - **ЭТАП 1 ТЗ v1.1 ЗАВЕРШЁН 14.09** (коммит da47dd4, Этап-контракты):
>   контракты §20 (news_items/медиа-мониторинг A1), §21 (channels),
>   §22 (inbounds/A4), §23 (роли); миграции 017-020 применены на проде
>   (0 ошибок); team: +U6 «Р.»/+U7 «П.» (admin, пароли заданы), U2 Оксана
>   content:approve, U4 content:edit, U1 оператор (inbound/clients/leads),
>   у U1 снесена мусорная пермиссия «бла бала»; docs/n8n/ — каркас
>   воркфлоу A1 (НЕ активировать до Этапа 2). Тест-данные удалены
>   (B917/B918, B2/B3, C6/C7, D9/D10; бэкап /root/backup_test_cleanup_
...
>   Осталось из Этапа 2: LLM-шлюз (генерация A2 / адаптация A3),
>   channels-CRUD в UI.
> - **Календарь публикаций 14.09**: вкладка в «Контенте» — месяц-сетка,
>   посты по scheduled_at, назначение слота кликом (approved→scheduled).
>   Проверено live. Этап 2 практически закрыт (кроме LLM-шлюза).
> - **§24 LLM-шлюз 14.09**: backend/common/llm.py (OpenRouter, ключ в .env,
>   LLM_MODEL=openai/gpt-4o-mini). A2 генерирует черновики LLM (fallback —
>   копия), A3 адаптирует текст под канал (POST /content/{id}/adapt, кнопка
>   «✨ Адаптировать» в редакторе). Live-генерация и адаптация проверены.
>   ЭТАП 2 ЗАКРЫТ (осталось: channels-CRUD в UI — косметика).
...
> - **§25 A6 «Мой день» 15.09**: GET/POST /v1/myday/digest (агрегат +
>   LLM-сводка + Telegram; тон A6_TONE), таймер agropilot-a6.timer
>   ежедневно 07:30 (CLI send_digest, u7-сервисный вход, U7_PASSWORD
>   в .env). Live: сводка доставлена (6 просроченных, 3 горячие).
>   Осталось из Этапа 3: реквизиты по ИНН — РЕШЕНИЕ ВЛАДЕЛЬЦА
>   (Контур API / DaData платно / открытый ЕГРЮЛ / ручной ввод).
> - **§26 реквизиты по ИНН 15.09 (вариант 2)**: миграция 026, POST
>   /v1/clients/{id}/requisites, разбор egrul.nalog.ru (JSON). Live:
>   реквизиты Сбера заполнены, плохой ИНН -> 422. ЭТАП 3 ЗАКРЫТ.
>   Следующий: Этап 4 — A5-артефакты (шаблоны/генерация), A7-чат,
>   каркас AgentCard/Prompt.
> - **§27 A5-артефакты 15.09**: миграция 027 (artifact_templates + сид
>   КП/письмо/договор; artifacts.body/client_id), GET/POST
>   /v1/artifacts/templates, POST /v1/artifacts/generate (контекст
>   сделки+клиента+ЕГРЮЛ, draft, missing-переменные). Live: КП по
>   орошению 12 га сгенерировано. Осталось Этап 4: A7-чат, AgentCard/
>   Prompt-каркас, UI-кнопка генерации на карточке сделки.
> - **§28 A7-чат-ассистент 15.09**: POST /v1/assistant/ask (контекстный
>   срез -> LLM, только по данным); чат ПЕТРУШКИ работает через A7.
>   Live: оба критериальных вопроса ТЗ (сделки/новости) отвечены по
>   реальным данным. Осталось Этап 4: AgentCard/Prompt-каркас,
>   UI-кнопки (артефакт на сделке, ИНН в клиенте); Этап 5 — приёмка.
> - **§29 реестр агентов 15.09**: миграция 028 (agent_cards a1..a7,
>   prompts с версиями), GET/PATCH /v1/agents, история промтов, PUT новой
>   версии (agents:manage; u1 — 403). Runtime A2/A3/A4/A7 на реестре.
>   Live: v2 промта A7 записана и подхвачена. Остаётся UI-редактор
>   промтов в «Настройках», кнопки артефакт/ИНН; затем Этап 5 — приёмка.
> - **UI-хвосты Этапа 4 закрыты 15.09**: «Настройки → Агенты» (редактор
>   промтов с версиями), ИНН-кнопка в карточке клиента, артефакт-кнопка
>   в карточке сделки — проверено в браузере. ЭТАП 4 ЗАКРЫТ, критерий
>   приёмки №7 выполнен полностью. Остался Этап 5 (сквозной прогон
>   критериев, инструкции, регресс).
> - **ЭТАП 5 / ТЗ v1.1 ЗАКРЫТО 15.09**: scripts/acceptance_test.py —
>   сквозной прогон 10/10 критериев приёмки PASS (конвейер с публикацией
>   в TG, inbound->A4->лид, ЕГРЮЛ, A5-КП, A6-сводка, A7, модульность,
>   промты-версии, лиды Bitrix24=916). Тест-артефакты прогона вычищены.
>   docs/ИНСТРУКЦИЯ_КОМАНДЫ_2026-09.md — редактору/оператору/разработчику.
>   Открытые хвосты (вне ТЗ v1.1): channels-CRUD, коннекторы входящих
>   (TG/почта -> inbounds), автотесты pytest, refresh n8n до 28.09.
> - **§30 «Стратегии»/«Цели» 16.09**: миграция 029 (strategy_directions
>   SD1/SD2, goals +direction/target/unit/current), CRUD направлений,
>   goals read-write с пересчётом прогресса, A1-фильтр по словам
>   направлений, живая вью «Стратегия». Live: цели привязаны,
>   PATCH прогресса работает. Фиксы: тихая потеря полей ORM goals,
>   UX генерации артефактов (список перезагружается).
> - **§31 хвосты 16.09**: channels-CRUD + publish по channel_id;
>   TG-коннектор входящих (tg_poller, таймер 5 мин, дедуп message_id,
>   служебный чат фильтруется); pytest 8/8 (venv сервера). Почтовый
>   коннектор входящих — при предоставлении mailbox-доступа (IMAP).
>   Осталось из крупного: дашборд агентов (отд. ТЗ), Instagram-коннектор,
>   email-входящие.
> - **§32 дашборд агентов 16.09**: ТЗ составлено (TZ_agents_dashboard_v1.0.md,
>   в СЕНТЯБРЬ/16.09.2026 — исходный файл отсутствовал), миграция 032
>   (run_logs), логирование всех семи агентов (LLM usage/cost + прогоны
>   A1/A5), GET /runs/all + /dashboard/summary, блок в UI-«Дашборде».
>   Live: A1/A7 залогированы, дашборд отрисован. Плюс: cache-busting
...
>   (логин проверен).
> - **§37 сегменты аудитории + рубрикатор 17.09 (коммиты e8d7c86+
>   5ad*фикс, миграция 035)**: A2 пишет языком сегмента (vine/grain/
>   greenhouse с CTA по концепту apilot92; 9 рубрик). Привязка: источник
>   (news наследует) -> пост, либо вручную; «Переписать под сегмент»
>   перегенерирует пост (версии сохраняются). Справочники — CRUD для
>   контент-мейкера в разделе «Контент». DoD: пост #6 vine->grain live.
>   ГРАБЛИ: (1) классы Pydantic объявлять ВЫШЕ роута (NameError на
>   импорте, пойман только серверным `python -c import backend.main` —
>   добавить в чек-лист деплоя); (2) /segments/rubrics до /{segment_id}.
> - **§35 TG-согласование постов + §36 UI медиа-мониторинга 17.09
>   (утро, коммиты 52a9f56+1ccfe1c, миграция 034)**: кнопки
>   «Опубликовать/Правка/Отложить» в TG владельца со SLA 15 мин/2 ч
>   (срочность авто по релевантности новости >= 0.7 + ручное
>   переопределение; канал выбирается при отправке, при отсутствии —
>   создание нового прямо в модалке). Callback-кнопки обрабатывает
>   tg_poller (сервисный u7, <= 5 мин; повтор — «уже обработано»); истёкший
>   SLA -> expired, красный бейдж в UI. «Отложить» = approved + слот в
>   календаре. §36: тулбар источников (+ добавить / удалить КАСКАДОМ с
>   новостями — DELETE /sources теперь полный), карточка новости
>   вертикальная (источник -> тема -> анонс с hover-фреймом краткого
>   содержания), кнопки операций в подвале. DoD: пост #5 отправлен
>   (urgent, канал id=3 «DoD-канал» с chat_id владельца, TG msg 2440);
>   V2-MIA-хвосты ИСКЛЮЧЕНЫ владельцем (фенофаза-ручная, комбинированные
>   правила, резюме->пост, ночной контроль) — в планы не брать.
> - **§34 MIA-погодный агро-консультант 17.09 (утро) — ЗАМЕНЯЕТ §17
>   field_alerts (решение владельца)**: Open-Meteo -> правила культуры/
>   фенофазы -> LLM-резюме «плюсы/минусы/рекомендации» -> Telegram.
>   Миграция 033 (crops/geo_points/crop_phases/crop_rules/
>   meteo_subscriptions/weather_runs + сиды: виноград с 12 фенофазами и
>   9 правилами, ЮБК-Ялта, подписка 24/72/120, агент a-mia + промт v1).
>   backend/meteo (provider/engine/routes/CLI), CRUD + POST /run + история;
>   UI-блок «🌤 Погода» в Мониторинге (карточки, кнопки горизонтов, чекбокс
>   TG, инлайн-добавление пунктов/культур/подписок). Таймеры
>   agropilot-meteo@{24,72,120} (06:00 МСК / пн-чт / пн; юниты в
>   deploy/systemd). Старый agropilot-mia.timer disable --now. DoD: прогоны
>   24/72/120 live, TG-сводка доставлена. Правила однофакторные; фенофаза
>   календарная; «резюме -> в пост A2» — v2.
> - **A1-n8n ПОЧИНЕН ОКОНЧАТЕЛЬНО 17.09 05:00**: execution 58 success,
>   автозапись a1 в run_logs (id=10) — конвейер n8n->API->дашборд живой.
...
## Запись 2026-08-28 — §18 Блок B «Справочник» реализован. План ТЗ §7 пройден целиком

Контракт `ca2bd84`, бэкенд `c8b2271`, фронт `e04720b`. DoD отмечен полностью,
проверка на проде после рестарта (MainPID 394897).

**Это последний шаг плана ТЗ §7.** Пройдены все блоки: D-1, E, C, D, A(1..7), B.
Разделов без backend-роутера не осталось.

### Что сделано

`backend/catalog` — read-model поверх 8 реестров, 47 листьев. Своих таблиц нет,
**миграций не добавлено** (в проекте по-прежнему 13 файлов, последняя — 015).

- `GET /v1/catalog/tree`: корень -> 8 веток со счётчиками
  (artifacts 21, content 0, deals 10, clients 7, goals 3, strategy_tasks 0,
  sources 1, team 5); `?node=<ветка>` -> листья; `sources` и `team` имеют
  промежуточный уровень (по type и role); неизвестный узел -> 404.
- `GET /v1/catalog/search`: ILIKE по заголовку и id во всех ветках,
  фильтр `type`, `q` короче 2 символов -> 422.
- Фронт: раздел «Справочник» — дерево с lazy-load и кэшем, превью листа,
  кнопка «Открыть», поиск. Пункт навигации, флаг `CATALOG_READY`.

### Разнородность моделей — учтено в коде

`Artifact` объявлен классическим `Column`, а не `mapped_column`. У `Goal` и
`TeamMember` поле `created_at` НЕ отображено в модели, хотя в БД оно есть.
Поэтому доступ к необязательным полям идёт через `getattr`, а сортировка для
таких веток деградирует с `created_at DESC, id DESC` на `id DESC`. Без этого
роутер падал бы на половине веток.

### Два осознанных отступления от ТЗ §5.5 (зафиксированы в §18.2 и §18.3)

1. **Поля `source_agent` в листе нет.** ТЗ требует указывать создателя
   (MIA / SMM / SPA / ПЕТРУШКА / человек). Такой колонки нет ни в одной таблице,
   а `owner_id` указывает на человека из `team`, а не на агента. Выводить нечем,
   эвристика была бы выдумкой под видом факта. Вместо него отдаётся `owner`
   там, где поле реально есть.
2. **Deep-link «в карточку» выполним частично.** Одиночные маршруты во фронте
   есть только у `client`, `deal`, `goal`, `project`. Для артефактов, контента,
   источников и пользователей ссылка ведёт в раздел-список. Превью честно пишет,
   что откроется: «Откроется карточка объекта» либо «У этого типа карточки нет —
   откроется раздел X». Создание недостающих карточек — отдельные контракты.

### Проверено на проде

Счётчики совпали с ожидаемыми; конверт пагинации; у всех листьев есть
`link.route`; поля `source_agent` в выдаче нет; `?node=bogus` -> 404; пустые
ветки видны с 0; поиск «КФХ» нашёл в двух ветках сразу (clients + deals);
`q` из 1 символа -> 422.

В UI: дерево рендерит 8 веток, lazy-load подгружает только раскрытую ветку
(после раскрытия «Клиенты» в кэше ровно один узел, 7 листьев), выбор листа
наполняет превью, «Открыть» на клиенте C1 действительно открыла карточку
«КФХ «Заря»». Отдельно проверен узел с пробелом и слэшем в id —
`team:Контент / SMM` разворачивается и отдаёт лист. Ошибок приложения в консоли
нет (три 404 — предсуществующие `reports` и `orchestrator/*`).

БД не изменена: количества строк во всех 8 таблицах прежние.

### Границы соблюдены (§18.7)

Лиды в дерево не включены (нет в перечне ТЗ; 918 записей утопили бы остальные
47). Палитра `Ctrl+K` не тронута — она целиком клиентская, перевод её на
`/v1/catalog/search` меняет поведение всей палитры и требует своего контракта.
Дерево read-only: drag-n-drop и переорганизации нет (открытый вопрос ТЗ §7 п.2
закрыт в пользу read-only, как и записано в самом ТЗ §5.5).

### Что остаётся открытым по проекту

- Починка поставщика наблюдений — `docs/MONITORING_PRODUCER_AUDIT.md`,
  решение о целевой архитектуре за владельцем.
- Недостающие карточки (артефакт, пост, источник, пользователь) — если нужен
  полноценный deep-link из «Справочника».
- Перевод палитры `Ctrl+K` на серверный поиск.
- Тестовые артефакты на проде: лиды `B917`, `B918`, клиенты `C6`, `C7`,
  сделки `D9`, `D10`, дела на `B2`/`B3` — ждут финальной зачистки.
