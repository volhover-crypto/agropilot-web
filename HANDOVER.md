...
- **КОРРЕКЦИЯ (2026-07-11)**: Форма API Календаря из §9 (GET /v1/calendar/events, POST /v1/calendar/events — 2 эндпоинта) устарела. Актуальный контракт M7 Calendar — CONTRACTS.md §1: 4 эндпоинта (GET/POST/PATCH/DELETE), реализован в backend/calendar/. Старые строки §9 оставлены как исторический артефакт — не удалять.
...
Добавлен раздел «Порог B→A» с критериями перехода Класс B → Класс A. Файл: CONTRACTS.md. Конкретные пороговые значения: V >= 10 (объём/количество), Q >= 0.80 (качество), окно = 30 дней. Переход B→A засчитывается при одновременном выполнении V >= 10 и Q >= 0.80 в окне 30 дней. Верификация: ранее содержимое было подтверждено сверкой по SHA 61d5d56 — блок присутствует и совпадал с согласованным diff (V >= 10, Q >= 0.80, окно 30 дней).

### M9-a — mock.objects.js: MOCKO.skills (SHA 3273b33) [x]
В MOCKO (js/mock.objects.js) добавлен блок const skills = [...] — 7 записей team_skills по контракту §3.1, каждая с полями {id, user_id, user_name, skill, level, note, updated_at}. Также добавлена константа EMPTY_MODEL и обновлён return (skills добавлен в экспорт). Файл: js/mock.objects.js. Верификация: ранее содержимое было подтверждено построчной сверкой блока skills (7 записей team_skills §3.1), EMPTY_MODEL и return по SHA 3273b33 — совпадало с 

### M9-b — skillMaturity() resolver + тестовые данные (SHA 76ad3d61) [x]
Реализован метод skillMaturity() в js/app.objects.js (между vMetrics() и ЧАНК 1.4/vMyDay4()): batch-агрегатор порога B→A. Логика: grade CONFIRM → засчитывается в V и числитель Q; AUTO → только в V; HINT → полный игнор; иное значение grade → console.warn + игнор. Окно — 30 дней от M.TODAY включительно. Порог: V>=10 AND Q>=0.80 → reached=true. user_id выводится через o.user_id либо через dealId→deals.owner(имя)→team[].name===owner→id. Возврат — batch map {user_id: {V, Q, reached}}.

В js/mock.objects.js добавлены тестовые данные для проверки порога: clients C2/C3/C4 dealsCount 1→2; deals D6 (C3/Дмитрий), D7 (C4/Сергей), D8 (C2/Марина, «Тепличный комплекс: доукомплектация»); owlSuggestions O1-O4 расширены полями user_id/date, добавлены O5-O30 (маркированы комментарием как синтетические).

Верификация: построчная сверка raw-содержимого по SHA 76ad3d6183d225b395d28ee9725fe669c4a3d950 подтвердила соответствие спецификации без отклонений. Пересчёт по фактическим данным: U1/Екатерина V=3/Q=0.67/false; U2/Оксана V=1/Q=1.0/false; U3/Дмитрий V=11/Q=0.91/true; U4/Марина V=2/Q=0.50/false; U5/Сергей V=12/Q=0.50/false.

### Итог по M9 (team_skills, порог B→A)
M9-doc [x] · M9-a [x] · M9-b [x] — все три подшага завершены и верифицированы построчной сверкой по Правилу 5. Коммит чек-пойнта: docs(HANDOVER): checkpoint M9 complete (M9-doc 61d5d56, M9-a 3273b33, M9-b 76ad3d6).
