...
backend M4 (PUT /v1/strategy + strategy/ модуль FastAPI) ИЛИ backend M9 skills (GET /v1/skills; POST /v1/skills/measure).
...
- **Следующий шаг**: backend M4 (strategy/ FastAPI модуль) ИЛИ backend M9 skills (`/v1/skills`, `/v1/skills/measure`).
...
- **Следующий шаг**: без изменений — backend M4 (strategy/) ИЛИ backend M9 skills; старт Этапа-2 (M10) — по отдельному решению заказчика.
