...
- ⚠️ api.js: проверить дубли VERSIONS_READY/SKILLS_READY после коммита b1fee6e — каждый флаг должен встречаться ровно 1 раз (grep -n "VERSIONS_READY\|SKILLS_READY" js/api.js).

## 7. Следующий шаг (ожидает решения пользователя)
M3 — перегруппировка навигации в 4 блока жизненного цикла клиента (block:1..4). Либо — backend M4 (PUT /v1/strategy + strategy/ модуль FastAPI).
...
- ⚠️ Открытый пункт: проверить дубли `VERSIONS_READY`/`SKILLS_READY` в api.js после b1fee6e

### Открыто до backend-подъёма (M4)
- `STRATEGY_READY: false` → активировать только после подъёма `PUT /v1/strategy` на реальном стенде
- Backend-модуль `backend/strategy/` (models.py + routes.py + migrations/001_create_strategy.sql) — не создан
...
- **Следующий шаг**: M3 (перегруппировка навигации block:1..4) ИЛИ backend M4 (strategy/ FastAPI модуль). Также: проверить дубли VERSIONS_READY/SKILLS_READY в api.js (grep -n).
