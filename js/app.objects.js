...
vSkillsMy() {
  ...
  async owlAsk() {
    ...
    // ======== M5-c: единый контракт HINT-объекта ПЕТРУШКИ (централизация) ========
    // Совместим со старой формой: поле action сохраняется (алиас kind); спец-поля
    // (taskTitle/taskType/packageName/clientId) дублируются в корень И в payload.
    _hintSeq: 0,
    _hintId() { this._hintSeq = (this._hintSeq + 1) % 100000; return 'O' + Date.now() + '_' + this._hintSeq; },
    makeHint(opt) {
      opt = opt || {};
      const kind = opt.kind || opt.action || 'task';
      const payload = opt.payload || {};
      return {
        id: opt.id || this._hintId(),
        dealId: opt.dealId || '',
        clientId: opt.clientId || payload.clientId || '',
        grade: opt.grade || 'HINT',
        text: opt.text || '',
        action: kind,
        kind: kind,
        okMsg: opt.okMsg || '',
        ...
        ts: this.M.TODAY,
        taskTitle: opt.taskTitle || payload.taskTitle || '',
        taskType: opt.taskType || payload.taskType || '',
        packageName: opt.packageName || payload.packageName || '',
        payload: payload,
      };
    },
...
window.AGL.createTask({ title: o.taskTitle || 'Задача', deal_id: d.id, status: 'active', priority: 'normal' }).then(res => { const nid = res && (res.id || (res.data && res.data.id)); if (nid) { const nt = this.M.tasks.find(t => t.id === tid); if (nt) nt.id = nid; this.persist(); this.render(); } }).catch(e => { this.toast('Задача не сохранена на сервере', 'err'); console.warn('[AGL] createTask failed:', e && e.message); });
        ...
        d.packageName = o.packageName || ''; msg = o.okMsg || `Упаковка применена к сделке`; this.logDeal(d, 'package', `ПЕТРУШКА: применена упаковка «${o.packageName || ''}»`, 'ПЕТРУШКА');
  ...
  vClients() {
    ...
    cliAddModal() {
      ...
      `, () => {
        ...
        this.owlPush({ source: 'client', dealId: '', clientId: c.id, grade: 'HINT', text: `Новый клиент «${this.clientName(c)}» — создать первую сделку-проект?`, kind: 'newdeal', okMsg: 'Открываю создание сделки' });
        this.go('client', c.id);
        return true;
      });
    },
...
window.AGL.createDeal({ name: title, client_id: clientId, stage: 'lead', finance: { budget: amount }, need_type: need }).then(res => { const nid = res && (res.id || (res.data && res.data.id)); if (nid) { d.id = nid; this.persist(); this.render(); } }).catch(e => { this.toast('Сделка не сохранена на сервере', 'err'); console.warn('[AGL] createDeal failed:', e && e.message); });
    ...
    dealSuggestPackage(d, cl) {
      ...
      const pk = this.M.packages.find(p => p.need === d.need && p.industry === ind)
        || this.M.packages.find(p => p.need === d.need);
      if (pk) {
        this.owlPush({
          source: 'deal', dealId: d.id, grade: 'HINT',
          text: `Подходит упаковка «${pk.name}» (от ${this.money(pk.priceFrom)}, ${pk.ready}) — применить к сделке?`,
          kind: 'package', packageName: pk.name,
          okMsg: `Упаковка «${pk.name}» применена к сделке`,
        });
      } else {
        this.owlPush({
          source: 'deal', dealId: d.id, grade: 'HINT',
          text: `Готовой упаковки по «${d.need}» нет — собрать концепцию?`,
          kind: 'task', taskTitle: `Собрать концепцию: ${d.need}`, taskType: 'kp',
          okMsg: 'Создана задача «Собрать концепцию»',
        });
      }
    },
...
if (this.apiMode && window.AGL && window.AGL.token) { const REV = { 'Зацепка':'lead','Оценка':'assess','Договор':'proposal','Проектирование':'deal','Реализация':'won','Проиграна':'lost','Сервис':'service','Отменена':'cancelled' }; const code = REV[stage] || stage; ds.forEach(d => window.AGL.setStage(d.id, code).catch(e => console.warn('[AGL] bulk setStage failed:', d.id, e && e.message))); }
    ...
    bulkDealOwner(name) {
      ...
      let n = 0;
      ds.forEach(d => { if (d.owner !== name) { d.owner = name; d.updated = this.M.TODAY; this.logDeal(d, 'stage', `Ответственный → ${name} (групповое)`, this.currentUser?.name || 'Оператор'); n++; } });
      this.selClear('deals');
    ...
    bulkDealProject(projId) {
      ...
      ds.forEach(d => {
        ...
        this.logDeal(d, 'stage', `Привязана к проекту «${p.title}» (групповое)`, this.currentUser?.name || 'Оператор');
      });
      this.selClear('deals');
    ...
    bulkTaskDeal(dealId) {
      ...
      this.logDeal(d, 'task', `Перепривязано задач: ${n} (групповое)`, this.currentUser?.name || 'Оператор');
      this.selClear('tasks');
    ...
    pkgSetReady(id, ready) {
      ...
      if (p.dealId) { const d = this.dealById(p.dealId); if (d) this.logDeal(d, 'package', `Упаковка «${p.name}» → ${ready}`, this.currentUser?.name || 'Оператор'); }
      this.render();
    },
    pkgFromDealModal() {
      const deals = this.M.deals;
      ...
      this.openModal('Новая упаковка из сделки', `
        ...
        () => {
          ...
          this.logDeal(d, 'package', `Создана упаковка «${name}» (черновик)`, this.currentUser?.name || 'Оператор');
    ...
    async postAdapt(id) {
      ...
      try {
        ...
        p.body = upd.body; p.title = upd.title;
        this.toast('Текст адаптирован LLM — старая версия в истории', 'ok');
      } catch (e) { this.toast(e.message || 'Адаптация не удалась', 'err'); }
      this.render();
    },
    ...
    inboxCreate(id) {
      ...
      if (it.suggest === 'deal') {
        ...
        this.logDeal(nd, 'create', 'Создана из входящих (' + it.src + ')', this.currentUser?.name || 'Оператор');
        go = ['deal', did];
        ...
        if (d) this.logDeal(d, 'task', 'Задача из входящих: ' + (f.title || ''), this.currentUser?.name || 'Оператор');
        go = ['tasks', null];
    ...
    artGenApply(dealId) {
      ...
      if (mode === 'pick') {
        ...
        this.logDeal(d, 'artifact', `ПЕТРУШКА: из шаблона «${tpl.title}» — ${type} «${title}»`, 'ПЕТРУШКА');
        this.persist();
      ...
      this.M.artifacts.push(na);
      this.logDeal(d, 'artifact', `ПЕТРУШКА: сгенерирован ${type} «${title}»`, 'ПЕТРУШКА');
      this.persist();
    ...
    srcScan(id) {
      ...
      if (!window.DEV_MOCK) {  // M2.7: в проде живой скан — через BFF (задел M5), без фабрикации mock-сигналов
        this.toast('Скан источника доступен только через BFF (демо-генерация отключена)', 'info');
        return;
      }
      this.M.signals.unshift({ id: sid, dealId: '', sev: 'info', text: `Новое из «${s.value}»: потенциальный запрос`, objectTitle: s.type });
      this.owlPush({ source: 'scan', dealId: '', grade: 'HINT', kind: 'newdeal', clientId: '', text: `Сигнал «${s.value}» — создать сделку / задачу / пост?`, okMsg: 'Открываю создание сделки' });
    ...
    signalAction(id) {
      ...
      const d = this.dealById(s.dealId);
      if (d) {
        const crit = s.sev === 'critical';
        this.owlPush({
          source: 'signal', dealId: d.id,
          grade: crit ? 'CONFIRM' : 'HINT',
          text: `По сигналу «${s.text}» — реагировать по сделке «${d.title}»?`,
          kind: 'task', taskTitle: `Реакция на сигнал: ${s.objectTitle}`,
          taskType: crit ? 'call' : 'email', okMsg: 'Создана задача по сигналу',
        });
      } else {
        this.owlPush({
          source: 'signal', dealId: '',
          grade: 'HINT', text: `Сигнал «${s.text}» — оформить как новую сделку?`,
          kind: 'newdeal', okMsg: 'Открываю создание сделки',
        });
      }
      this.owl.open = true;
