...
if (window.DEV_MOCK) {  // M2.6-b: сид демо-истории только в mock-режиме
    ...
    moveDeal(dealId, stage) {
      ...
      this.logDeal(d, 'stage', `Стадия: ${from} → ${stage} (перетаскивание)`, 'Оператор');
    ...
    bulkDealStage(stage) {
      ...
      let n = 0;
      ds.forEach(d => { if (d.stage !== stage) { const from = d.stage; d.stage = stage; d.updated = this.M.TODAY; this.logDeal(d, 'stage', `Стадия: ${from} → ${stage} (групповое)`, 'Оператор'); n++; } });
      this.selClear('deals');
    ...
    bulkDealOwner(name) {
      ...
      let n = 0;
      ds.forEach(d => { if (d.owner !== name) { d.owner = name; d.updated = this.M.TODAY; this.logDeal(d, 'stage', `Ответственный → ${name} (групповое)`, 'Оператор'); n++; } });
      this.selClear('deals');
    ...
    bulkDealProject(projId) {
      ...
      ds.forEach(d => {
        ...
        this.logDeal(d, 'stage', `Привязана к проекту «${p.title}» (групповое)`, 'Оператор');
      });
      this.selClear('deals');
    ...
    bulkTaskComplete() {
      ...
      ts.forEach(t => {
        ...
        t.status = 'done';
        const d = this.dealById(t.dealId); if (d) this.logDeal(d, 'task', `Задача «${t.title}» завершена (групповое)`, 'Оператор');
        done++;
      });
      this.selClear('tasks');
    ...
    bulkTaskDeal(dealId) {
      ...
      this.logDeal(d, 'task', `Перепривязано задач: ${n} (групповое)`, 'Оператор');
      this.selClear('tasks');
    ...
    taskComplete(id) {
      ...
      t.status = 'done';
      const d = this.dealById(t.dealId); if (d) this.logDeal(d, 'task', `Задача «${t.title}» завершена`, 'Оператор');
    ...
    pkgSetReady(id, ready) {
      ...
      if (p.dealId) { const d = this.dealById(p.dealId); if (d) this.logDeal(d, 'package', `Упаковка «${p.name}» → ${ready}`, 'Оператор'); }
      this.render();
    },
    pkgFromDealModal() {
      const deals = this.M.deals;
      ...
      this.openModal('Новая упаковка из сделки', `
        ...
        () => {
          ...
          this.logDeal(d, 'package', `Создана упаковка «${name}» (черновик)`, 'Оператор');
    ...
    inboxCreate(id) {
      ...
      if (it.suggest === 'deal') {
        ...
        this.logDeal(nd, 'create', 'Создана из входящих (' + it.src + ')', 'Оператор');
        go = ['deal', did];
        ...
        if (d) this.logDeal(d, 'task', 'Задача из входящих: ' + (f.title || ''), 'Оператор');
        go = ['tasks', null];
