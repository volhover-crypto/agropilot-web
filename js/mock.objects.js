...
inbox: [], team: [],
    strategy: { id: '', title: '', horizon: '', scenarios: [], updated_at: '', updated_by: '' },
...
// --- СТРАТЕГИЯ (M4: Strategy; CONTRACTS.md §4) ---
    // Единый глобальный объект. id фиксирован: 'strategy_main'.
    // scenarios[]: 2..4 сценария, каждый с indicators[] и action_lines[].
    // Поле directions[] удалено — модель сменилась на scenarios[].
    const strategy = {
      id: 'strategy_main',
  ...
  const posts = [
    ...
    // терминальные статусы — демонстрация архивов публикаций (П.6)
    { id: 'PST6', kind: 'Обучающий', icon: '🌾', title: 'Подкормка озимых: сроки по фенофазе', status: 'опубликован', slot: '2026-06-10 09:00', channel: 'Telegram',
      body: '🌾 Разбираем подкормку озимых по фенофазам — от кущения до выхода в трубку.',
      hashtags: '#озимые #подкормка #агроном', media: 'Таблица норм' },
    { id: 'PST7', kind: 'Рынок',     icon: '📉', title: 'Тендер на теплицы: разбор условий', status: 'отклонён', slot: '', channel: 'Telegram',
      body: '📉 Разбор условий тендера на строительство тепличного комплекса.',
      hashtags: '#тендер #теплицы', media: 'Скриншот документации' },
  ];
